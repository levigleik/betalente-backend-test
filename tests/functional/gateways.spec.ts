...
test.group('Gateways', (group) => {
  ...
  test('toggle gateway active status', async ({ client, assert }) => {
    ...
    const response = await client.patch(`/v1/gateways/${g.id}/toggle`).loginAs(authUser)

    response.assertStatus(200)
    const body = response.body() as any
    assert.equal(body.message, 'Gateway atualizado com sucesso')
    assert.isTrue(body.data.isActive) // turned true

    const dbG = await Gateway.findOrFail(g.id)
    assert.isTrue(dbG.isActive)
  })
  ...
  test('update gateway priority', async ({ client, assert }) => {
    ...
    const response = await client.patch(`/v1/gateways/${g.id}/priority`).json(payload).loginAs(authUser)
