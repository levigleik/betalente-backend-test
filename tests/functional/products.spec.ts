...
test.group('Products', (group) => {
  ...
  test('create product', async ({ client, assert }) => {
    ...
    const response = await client.post('/v1/products').json(payload).loginAs(authUser)
  ...
  test('update product', async ({ client, assert }) => {
    ...
    const response = await client.put(`/v1/products/${p.id}`).json(payload).loginAs(authUser)
  ...
  test('delete product', async ({ client, assert }) => {
    ...
    const response = await client.delete(`/v1/products/${p.id}`).loginAs(authUser)
